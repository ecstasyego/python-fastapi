...
@app.get("/src")
def response(params: Request):
    params = dict(params.query_params)

    serverResponse = dict()
    serverResponse["rspns00"] = params["param00"]
    serverResponse["rspns01"] = params["param01"]
    return serverResponse
```
```bash
$ uvicorn script:app --host 0.0.0.0 --port 8000 --reload
...
@app.get("/src/{param00}")
def response(param00:str, params: Request):
    params = dict(params.query_params)

    serverResponse = dict()
    serverResponse["rspns00"] = param00
    serverResponse["rspns01"] = params["param01"]
    serverResponse["rspns02"] = params["param02"]
    return serverResponse
```
```bash
$ uvicorn script:app --host 0.0.0.0 --port 8000 --reload
...
interface ApiService {
    @GET("/src/{param00}")
    suspend fun request(@Path("param00") param00:String , @QueryMap params: Map<String, @JvmSuppressWildcards Any>): ServerResponse
}
...
fun main() = runBlocking {
    try {
        api.request( "Toy", mapOf("param01" to "Dao", "param02" to 20) )
    } catch (e: Exception) {
        e.printStackTrace()
        println("${e.message}")
    }
